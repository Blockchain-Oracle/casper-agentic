...
| Typed data follows Casper exact/CSPR.click shape | `buildCSPRClickPaymentIntentParams()` returns `TransferWithAuthorization`, Casper domain fields, domain types, `returnHashArtifacts: true`, and `0x`-prefixed domain/message values matching the local Casper x402 exact scheme and CSPR.click reference. |
